"""
homogeneous_gcn.py — Homogeneous GCN baseline.

Architecture:
  1. ``HeteroToHomoAdapter`` — project each node type to shared ``hidden_dim``
     and build a single homogeneous ``edge_index``.
  2. Stack of ``GCNConv`` layers (PyG) — standard graph convolution that is
     *not* relation-type aware.
...
Reference:
  - Kipf & Welling, ICLR 2017. "Semi-Supervised Classification with Graph
...
class HomogeneousGCN(nn.Module):
    ...
    Args:
        ...
        num_layers: Number of GCNConv layers (default 3).
        dropout: Dropout rate after each activation (default 0.2).
        activation: Activation name (default ``"relu"``).
        use_batch_norm: Whether decoder heads use BatchNorm (default True).
