#!/usr/bin/env python3
"""
train_baseline.py — Unified training entry point for baseline suite.
...
Supports:
  ...
  - ``ms_hgt``       — MSHGTBaseline (HGT + macro anchor graph + cross-scale fusion)
...
def _get_input_construction_desc(model_name: str, model_cfg: Dict) -> str:
    ...
    if model_name == "mlp":
        ...
        return (
            "OursBaselineV2 (EA-HGNN v2): Type-specific Linear projections per node type "
            "(mesh_node 15-dim, beam_element 11-dim, plate_element 6-dim) to shared "
            "hidden_dim. Membership edges (4 types) use SAGEConv (relation-specific). "
            "Structural_link edge uses StructuralLinkConvV2 with residual gate "
            "(1 + scale*tanh(MLP(edge_attr))), root/self path (lin_root), "
            "and edge bias OFF by default. Sum aggregation per node type, "
            "activation + dropout + LayerNorm. "
            "Shared dual decoder: MLPHead for disp (6) and force (12). "
            "No macro anchor, no physics loss, no UQ — Stage 3 boundary."
        )
    elif model_name == "ms_hgt":
        return (
            "MSHGTBaseline (Multi-Scale HGT): Type-specific Linear projections "
            "(mesh_node 15-dim, beam_element 11-dim, plate_element 6-dim) to shared "
            "hidden_dim. HGTConv typed attention on 5 edge types. "
            "MacroAnchorPool: geometric segments along X-axis (n_segments=12), "
            "mean-pool mesh hidden, concat static features (center_x, log node count), "
            "shared MacroGNN (2-layer SAGEConv) on sequential chain macro graph. "
            "Per-layer CrossScaleFusion (gated residual). "
            "MLPHead decoders for disp (6) and force (12). "
            "No physics loss, no UQ, no edge_attr gating — Stage 4 boundary."
        )
    elif model_name == "ms_hgt_additive":
        return (
            "MSHGTBaseline (Multi-Scale HGT) with additive fusion (ablation). "
            "Same architecture as ms_hgt, but cross-scale fusion uses h + macro_unpooled "
            "instead of gated residual. "
            "Used for ablation comparison: does gating matter?"
        )
    return ""
